...
def preprocess_data(first_date_to_keep = date.today() - timedelta(days=365), companies_to_keep: list = None):
    ...
    dta = dta.dropna(subset=['consumer_complaint_narrative'])
    ...
    #make first_date_to_keep a date object instead of string
    if isinstance(first_date_to_keep, str):
        first_date_to_keep = pd.to_datetime(first_date_to_keep).date()
    ...
    #keep only specified companies
    dta = dta[dta.company.isin(companies_to_keep)]
